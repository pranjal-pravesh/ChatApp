    ...
    <Stack sx={{ width:'60px', alignItems:'center', height:'100svh', paddingTop:'30px', justifyContent:'space-between',
            ...
            {sidebarIcons.map((item)=>{
                ...
                <Box sx={{backgroundColor:'#4cb5f9' , width:'40px', height:'40px', borderRadius:'10px', margin:'24px 0px', transitionDuration:'150ms'}} key={item.index}>
                    <IconButton sx={{color:'white'}} key={item.index} onClick={()=>dispatch(changeTab(item.name))}>                    
                            {item.icon}                  
                    </IconButton>
                </Box></Link>
                )}
                ...
                return <Link to={item.link}><Box sx={{backgroundColor:'none' , width:'40px', height:'40px', borderRadius:'50%', margin:'24px 0px', transitionDuration:'150ms'}} key={item.index}>
                    <IconButton sx={{color:theme==='light'?'black':'white'}} key={item.index} onClick={()=>dispatch(changeTab(item.name))}>                    
                            {item.icon}                  
                    </IconButton>
                </Box></Link>    
                }
            ...
            {/**renders the gear icon with selecting condition */}
             {(selected==='settings')?(
                ...
                <Box sx={{backgroundColor:'#4cb5f9' , width:'40px', height:'40px', borderRadius:'10px', margin:'24px 0px'}}>
                    <IconButton sx={{color:'white'}} onClick={()=>dispatch(changeTab('settings'))}>                    
                ...
                <IconButton onClick={()=>dispatch(changeTab('settings'))} sx={{color:theme==='light'?'black':'white'}}>                    
                    <Gear size={24} weight="light"/>                  
                </IconButton>
                </Box> </Link>
                )      
